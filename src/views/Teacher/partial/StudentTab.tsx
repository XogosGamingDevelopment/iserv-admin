...
type ChildTabProps = {
  ...
  approveStudent: (item_id: string,approvalstatusname?: string) => void;
  deleteStudent: (item_id: string) => void;
};

const StudentTab: React.FC<ChildTabProps> = ({ user, approveStudent, deleteStudent }) => {
	...
	<div className="card card-dashed h-xl-100 flex-row flex-stack flex-wrap p-6">
		...
		<div className="d-flex align-items-center py-2">
		  {hasAuthorization('admin_approval', 'student') && (
		    <TooltipWrapper title="Update Approval Status" placement="top">
		     <span className="btn btn-sm btn-icon btn-light btn-active-light-warning me-2" onClick={() => approveStudent(_id,approvalstatusname)}>
		...
		{hasAuthorization('delete', 'parent') && (
		  <TooltipWrapper title="Delete Child" placement="top">
		    <span className="btn btn-sm btn-icon btn-light btn-active-light-danger" onClick={() => deleteStudent(_id)}>
