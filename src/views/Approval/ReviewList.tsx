...
import SearchForm from "./review-list-partial/SearchForm";
...
const ReviewList: React.FC = () => {
  ...
  const { changeApprovalStatus, deleteItem, archiveUnarchiveRating } = useCommonActions();
  ...
  const getListData = React.useCallback(async () => {
    ...
    try {
      const response: any = await axiosInstance({
        url: "rating-feddbacks/pending-review-approval-list",
        method: "GET",
        data: {
  ...
  const updateApprovalStatus = async (
    ...
    changeApprovalStatus({
      itemId,
      url: "rating-feddbacks/approve-disapprove-rating",
  ...
  /*Change Admin approval status*/
  const markArchiveUnarchive = async (itemId?: string, is_archived?: boolean | number) => {
    ...
    archiveUnarchiveRating(itemId, new_status, 'rating-feddbacks/archive-unarchive-rating', message, getListData);
  };
  ...
  return (
    <>
      ...
      <PageToolbar title="Event Review Approval List" />
        ...
        <div className={`card h-70vh ${isLoading ? "overlay overlay-block" : ""}`}>
          <div className="card-body pt-0 pb-0 overflow-auto">
            <div className="dt-container dt-bootstrap5 dt-empty-footer">
              <table className="table table-hover table-row-bordered table-row-solid gy-4 gx-4 align-middle fw-bold w-100">
                      ...
                      <tr key={item._id}>
                        ...
                        <td className="text-center">
                          <div className="d-flex">
						    {hasAuthorization("archive_unarchive", "review") && (
                              <TooltipWrapper
                                title={`Mark ${item?.is_archived ? 'Un-Archive' : 'Archive'}`}
                                placement="top"
                              >
                                <span
                                ...
                                >
                                  <i className="ki-outline ki-file-down text-warning fs-2"></i>
                                </span>
                              </TooltipWrapper>
                            )}
