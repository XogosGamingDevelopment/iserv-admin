...
import CreateUpdateSetting from "./CreateUpdateSetting";
import useCommonActions from "../../hooks/useCommonActions";
import { SettingSearchFormState, Setting } from "../../types/interfaces";
...
const List: React.FC = () => {
  ...
  const [searchForm, setSearchForm] = useState<SettingSearchFormState>({
    search_string: "",
    status: "",
  });
  const [results, setResults] = useState<Setting[] | null>(null);
  ...
  const getListData = React.useCallback(async () => {
    ...
    try {
      ...
      setResults(response.data.settinglists.docs);
      setTotalItems(response.data.settinglists.totalDocs);
    } catch (error: any) {
  ...
  const changeSettingStatus = async (itemid?: string, status?: number) => {
  ...
  const deleteSetting = async (itemid?: string) => {
    const message = `Are you sure you want to delete the Setting?`;
    deleteItem(`settings/delete/${itemid}`, message, getListData);
  };
  ...
  return (
    <>
      <div className="d-flex justify-content-between">
        ...
        <PageToolbar title="Setting List" />
        <div className="d-flex align-items-center">
          <span
            className="btn btn-dark btn-sm fw-bold fs-6 px-4 py-2"
            onClick={() =>
              handleOpenModal(
                <CreateUpdateSetting setting={null} getListData={getListData} />
              )
            }
          >
            Create Setting
                      ...
                      <tr key={item._id}>
                        ...
                        <td className="">
                          <div className="d-flex">
                            ...
                            {hasAuthorization("update_status", "setting") && (
                              <TooltipWrapper
                                title="Update Status"
                                placement="top"
                              >
                                <span
                                  className="btn btn-sm btn-icon btn-light btn-active-light-warning me-2"
                                  onClick={() =>
                                    changeSettingStatus(item?._id, item?.status)
                                  }
                                >
                            ...
                            {hasAuthorization("delete", "setting") && (
